...
\begin{figure}[h]
    ...
    \caption{[Od lewej] Barwowy obraz wejściowy 256x256, obraz po przesunięciu o współrzędnych \(x_{min}=70\), \(x_{max}=140\), \(y_{min}=160\), \(y_{max}=200\)}%
    \label{fig:geo_after_grey1}%
\end{figure}
\FloatBarrier
\begin{figure}[h]
    ...
    \caption{[Od lewej] Barwowy obraz wejściowy 1025x1025, obraz po przesunięciu o współrzędnych \(x_{min}=600\), \(x_{max}=1000\), \(y_{min}=160\), \(y_{max}=400\)}%
    \label{fig:geo_after_grey1}%
\end{figure}
\FloatBarrier
\subsection*{Kod źródłowy algorytmu}
